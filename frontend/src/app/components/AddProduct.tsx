...
type FormData = {
  ...
  price: string; quantity: string; minStock: string; description: string;
};
...
export function AddProduct() {
  ...
  return (
    <div style={{ padding: "28px 32px" }}>
      ...
      {success && (
        <div
          style={{ padding: "14px 18px", borderRadius: "10px", background: "#f0fdf4", border: "1px solid #bbf7d0", color: "#15803d", fontSize: "0.85rem", fontWeight: 500, marginBottom: "20px", display: "flex", alignItems: "center", gap: "8px" }}
        >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: "20px" }}>
        {/* Main form card */}
        <div
          style={{ background: "white", borderRadius: "14px", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.05)", overflow: "hidden" }}
        >
          {/* Card header */}
          <div
            style={{ display: "flex", alignItems: "center", gap: "12px", padding: "18px 24px", borderBottom: "1px solid #f1f5f9", borderLeft: "4px solid #7c3aed" }}
          >
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div style={{ padding: "24px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              ...
              <Field id="name" label="Product Name" required error={errors.name}>
                <input
                  id="name" type="text" value={form.name} placeholder="e.g. Wireless Keyboard"
                  onFocus={() => setFocus("name")} onBlur={() => setFocus(null)} onChange={set("name")}
                  style={INPUT(focus === "name", !!errors.name)}
                />
              </Field>
              ...
              <Field id="sku" label="SKU / Product Code" required error={errors.sku}>
                <input
                  id="sku" type="text" value={form.sku} placeholder="e.g. WK-001"
                  onFocus={() => setFocus("sku")} onBlur={() => setFocus(null)} onChange={set("sku")}
                  style={INPUT(focus === "sku", !!errors.sku)}
                />
              </Field>
              ...
              <Field id="category" label="Category" required error={errors.category}>
                <div style={{ position: "relative" }}>
                  <select
                    ...
                    onFocus={() => setFocus("category")} onBlur={() => setFocus(null)} onChange={set("category")}
                    style={{ ...INPUT(focus === "category", !!errors.category), appearance: "none", paddingRight: "34px", cursor: "pointer" }}
                  >
              ...
              <Field id="supplier" label="Supplier" required error={errors.supplier}>
                <div style={{ position: "relative" }}>
                  <select
                    ...
                    onFocus={() => setFocus("supplier")} onBlur={() => setFocus(null)} onChange={set("supplier")}
                    style={{ ...INPUT(focus === "supplier", !!errors.supplier), appearance: "none", paddingRight: "34px", cursor: "pointer" }}
                  >
              ...
              <Field id="price" label="Price (USD)" required error={errors.price}>
                <div style={{ position: "relative" }}>
                  ...
                  <input
                    id="price" type="number" min="0" step="0.01" value={form.price} placeholder="0.00"
                    onFocus={() => setFocus("price")} onBlur={() => setFocus(null)} onChange={set("price")}
                    style={{ ...INPUT(focus === "price", !!errors.price), paddingLeft: "24px" }}
                  />
                </div>
              </Field>
              ...
              <Field id="quantity" label="Initial Quantity" required error={errors.quantity}>
                <input
                  id="quantity" type="number" min="0" value={form.quantity} placeholder="0"
                  onFocus={() => setFocus("quantity")} onBlur={() => setFocus(null)} onChange={set("quantity")}
                  style={INPUT(focus === "quantity", !!errors.quantity)}
                />
              </Field>
              ...
              <Field id="minStock" label="Minimum Stock Level" required error={errors.minStock}>
                <input
                  id="minStock" type="number" min="0" value={form.minStock} placeholder="e.g. 10"
                  onFocus={() => setFocus("minStock")} onBlur={() => setFocus(null)} onChange={set("minStock")}
                  style={INPUT(focus === "minStock", !!errors.minStock)}
                />
              </Field>
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                <Field id="description" label="Description (optional)">
                  <textarea
                    ...
                    style={{ ...INPUT(focus === "description", false), resize: "none" as const }}
                  />
                </Field>
              </div>
            </div>
            ...
            {/* Actions */}
            <div
              style={{ display: "flex", gap: "12px", padding: "16px 24px", borderTop: "1px solid #f1f5f9", background: "#fafafa" }}
            >
              <button
                type="submit"
              ...
              <button
                ...
                style={{ padding: "10px 20px", borderRadius: "10px", background: "transparent", color: "#94a3b8", border: "none", cursor: "pointer", fontSize: "0.88rem" }}
              >
                Cancel
              </button>
            </div>
        ...
        {/* Sidebar tips card */}
        <div
          style={{ background: "white", borderRadius: "14px", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.05)", padding: "22px", height: "fit-content" }}
        >
          ...
          <div
            style={{ padding: "12px", borderRadius: "9px", background: "#f5f3ff", border: "1px solid #ddd6fe" }}
          >
            <div style={{ color: "#6d28d9", fontSize: "0.78rem", fontWeight: 600, marginBottom: "3px" }}>⚠️ Note</div>
            <div style={{ color: "#4c1d95", fontSize: "0.75rem", lineHeight: 1.6 }}>
              Fields marked with <span style={{ color: "#7c3aed" }}>*</span> are required before saving.
            </div>
          </div>
        </div>
      </div>
